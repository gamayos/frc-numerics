...
function hopf(P, NS){                       // NS: a fixed non-square mod P
  ...
  // the Borel: the stabilizer of infinity = the affine group x -> ax + b
  const B = new Set();
  ...
  // the fiber coordinate is the horizon circle: t -> t(infinity) bijects
  // T with P1, the shell's cells plus the point at infinity
...
for (const [P, NS] of [[13, 2], [5, 2]]){
  ...
  ok(`p = ${P}: the fiber coordinate is the horizon circle: t -> `+
     `t(infinity) bijects the boost torus with P1, the cells plus the `+
     `point at infinity`, H.orbP1 === P + 1);
}
