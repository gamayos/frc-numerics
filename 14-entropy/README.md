...
Self-contained: no path outside this directory is referenced by either script.
Run from a clean clone; exit status 0 iff all asserted checks pass.

## Run

```
python3 estimate_S.py                  # numeric blocks (standalone)
python3 estimate_S.py <path/main.tex>  # + continuum-token census gate on the
                                       #   manuscript source, passed explicitly
