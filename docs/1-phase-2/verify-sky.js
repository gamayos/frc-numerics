...
const eqPH = (u, v) => u[0] === v[0] && u[1] === v[1];
{
  ok('the two covers are of this universe: C24 = 2 x 12 doubles the '+
     ...
     'the totality, and coprime as covers, gcd(13, 24) = 1',
...
// ---- 4. the radial ladder: three fibers ----
{
  ...
  // states (2c - 1 is odd), the complementary six at the next chronon:
  // the chronon-parity pair completes the affine translation part, and
  // the origin stabilizer -- the pure drive, the observer's clock --
  // completes the group
  let six = true;
  for (let c = 1; c < P; c++){
    const ex = new Set();
  ...
  ok('the totality decomposed: per cell the three shells x two routes '+
     'display six distinct drive-states (2c - 1 odd), the complementary '+
     'six at the next chronon: 2 x 72 = 144 = the affine translation '+
     'part; the origin stabilizer, the pure drive, is the observer\'s '+
     'own clock, 12 null events: 144 + 12 = 156 = p(p - 1), the Borel '+
     'with nothing left over (00:Y3)',
     six && 2 * 72 + 12 === P * (P - 1) &&
     3 * 48 + 12 === P * (P - 1) && 48 === 4 * 12);
}
